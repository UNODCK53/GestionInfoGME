...
class cusuarios extends cTable {
	...
	function __construct() {
		...
		// rol
		$this->rol = new cField('usuarios', 'usuarios', 'x_rol', 'rol', '`rol`', '`rol`', 3, -1, FALSE, '`rol`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->rol->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['rol'] = &$this->rol;

		// Entidad
		$this->Entidad = new cField('usuarios', 'usuarios', 'x_Entidad', 'Entidad', '`Entidad`', '`Entidad`', 200, -1, FALSE, '`Entidad`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Entidad'] = &$this->Entidad;

		// Cargo
		$this->Cargo = new cField('usuarios', 'usuarios', 'x_Cargo', 'Cargo', '`Cargo`', '`Cargo`', 200, -1, FALSE, '`Cargo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->rol->Exportable) $Doc->ExportCaption($this->rol);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->rol->Exportable) $Doc->ExportField($this->rol);
