...
function valores ($ano,$fase,$punto) {
	...
	{
		$sql_ano=" where SUBSTR(`FECHA_REPORT`,1,4)=".$ano."";
	}
	
	$fechas=mysql_query("select CAST(CONCAT(SUBSTR(`FECHA_REPORT`,1,4),'-',`MES`,'-',`DIA`,' 00:00:00')as date) as fechas FROM gestioninfogme.info_diario " .$sql_ano.$sql_fase.$sql_PE." group by fechas ORDER BY `fechas` ASC");
