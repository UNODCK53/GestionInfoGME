...
function valores ($ano,$fase,$punto) {
	...
	if ($fase!=99 && $ano != 99){
				...
				$ha = mysql_query("select (CASE NOM_PE WHEN NOM_PE !='Otro' THEN Otro_PE ELSE NOM_PE END ) as NOM_PE,CAST(CONCAT(SUBSTR(`FECHA_REPORT`,1,4),'-',`MES`,'-',`DIA`,' 00:00:00')as date) as fecha,convert(sum(T_erradi), decimal(3,2)) as ha FROM gestioninfogme.info_diario  WHERE SUBSTR(`FECHA_REPORT`,1,4)=".$ano."  and `FASE`='".$fase."'".$sql_PE."  group by NOM_PE,fecha order by (CASE NOM_PE WHEN NOM_PE !='Otro' THEN Otro_PE ELSE NOM_PE END ) asc,fecha asc");
				...
				$ha = mysql_query("select (CASE NOM_PE WHEN NOM_PE !='Otro' THEN Otro_PE ELSE NOM_PE END ) as NOM_PE,CAST(CONCAT(SUBSTR(`FECHA_REPORT`,1,4),'-',`MES`,'-',`DIA`,' 00:00:00')as date) as fecha,convert(sum(T_erradi), decimal(3,2)) as ha FROM gestioninfogme.info_diario  WHERE SUBSTR(`FECHA_REPORT`,1,4)=".$ano.$sql_PE." group by (CASE NOM_PE WHEN NOM_PE !='Otro' THEN Otro_PE ELSE NOM_PE END ),fecha order by (CASE NOM_PE WHEN NOM_PE !='Otro' THEN Otro_PE ELSE NOM_PE END ) asc,fecha asc");
