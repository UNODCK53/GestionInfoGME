...
<?php include_once "accidentesinfo.php" ?>
...
$accidentes_list = NULL; // Initialize page object first

class caccidentes_list extends caccidentes {
	...
	// Table name
	var $TableName = 'accidentes';

	// Page object name
	var $PageObjName = 'accidentes_list';

	// Grid form hidden field names
	var $FormName = 'faccidenteslist';
	...
	function __construct() {
		...
		// Table object (accidentes)
		if (!isset($GLOBALS["accidentes"]) || get_class($GLOBALS["accidentes"]) == "caccidentes") {
			$GLOBALS["accidentes"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["accidentes"];
		}
		...
		$this->AddUrl = "accidentesadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "accidentesdelete.php";
		$this->MultiUpdateUrl = "accidentesupdate.php";
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 'accidentes', TRUE);
	...
	function Page_Terminate($url = "") {
		...
		// Export
		global $EW_EXPORT, $accidentes;
		if ($this->CustomExport <> "" && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, $EW_EXPORT)) {
				$sContent = ob_get_contents();
			if ($gsExportFile == "") $gsExportFile = $this->TableVar;
			$class = $EW_EXPORT[$this->CustomExport];
			if (class_exists($class)) {
				$doc = new $class($accidentes);
				$doc->Text = $sContent;
	...
	function RenderOtherOptions() {
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->CustomActions as $action => $name) {
				...
				$item->Body = "<a class=\"ewAction ewCustomAction\" href=\"\" onclick=\"ew_SubmitSelected(document.faccidenteslist, '" . ew_CurrentUrl() . "', null, '" . $action . "');return false;\">" . $name . "</a>";
			}
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_accidentes\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_accidentes',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.faccidenteslist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($accidentes_list)) $accidentes_list = new caccidentes_list();

// Page init
$accidentes_list->Page_Init();

// Page main
$accidentes_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$accidentes_list->Page_Render();
?>
<?php include_once "header.php" ?>
<?php if ($accidentes->Export == "") { ?>
<script type="text/javascript">

// Page object
var accidentes_list = new ew_Page("accidentes_list");
accidentes_list.PageID = "list"; // Page ID
var EW_PAGE_ID = accidentes_list.PageID; // For backward compatibility

// Form object
var faccidenteslist = new ew_Form("faccidenteslist");
faccidenteslist.FormKeyCountName = '<?php echo $accidentes_list->FormKeyCountName ?>';

// Form_CustomValidate event
faccidenteslist.Form_CustomValidate = 
...
faccidenteslist.ValidateRequired = true;
<?php } else { ?>
faccidenteslist.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php if ($accidentes_list->TotalRecs > 0 && $accidentes_list->ExportOptions->Visible()) { ?>
...
<?php
	$bSelectLimit = EW_SELECT_LIMIT;
	if ($bSelectLimit) {
		if ($accidentes_list->TotalRecs <= 0)
			$accidentes_list->TotalRecs = $accidentes->SelectRecordCount();
	} else {
		if (!$accidentes_list->Recordset && ($accidentes_list->Recordset = $accidentes_list->LoadRecordset()))
			$accidentes_list->TotalRecs = $accidentes_list->Recordset->RecordCount();
	}
	$accidentes_list->StartRec = 1;
	if ($accidentes_list->DisplayRecs <= 0 || ($accidentes->Export <> "" && $accidentes->ExportAll)) // Display all records
		$accidentes_list->DisplayRecs = $accidentes_list->TotalRecs;
	if (!($accidentes->Export <> "" && $accidentes->ExportAll))
		$accidentes_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$accidentes_list->Recordset = $accidentes_list->LoadRecordset($accidentes_list->StartRec-1, $accidentes_list->DisplayRecs);

	// Set no record found message
	if ($accidentes->CurrentAction == "" && $accidentes_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$accidentes_list->setWarningMessage($Language->Phrase("NoPermission"));
		if ($accidentes_list->SearchWhere == "0=101")
			$accidentes_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$accidentes_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$accidentes_list->RenderOtherOptions();
?>
<?php $accidentes_list->ShowPageHeader(); ?>
<?php
$accidentes_list->ShowMessage();
?>
...
<?php
	foreach ($accidentes_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
		$option->Render("body", "");
	}
?>
...
<?php if ($accidentes->Export == "") { ?>
<script type="text/javascript">
faccidenteslist.Init();
</script>
<?php } ?>
<?php
$accidentes_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($accidentes->Export == "") { ?>
...
<?php
$accidentes_list->Page_Terminate();
?>
