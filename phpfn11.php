...
function ew_AutoLoad($class) {
	...
	if (substr($class, 0, 1) == "c") {
		if ($class == "cgestioninfogme_db") {
			$file = "gestioninfogmedb.php";
		} else {
		$fn = "%cls%info.php";
...
function CurrentProjectID() {
	...
	return "{D6213859-7C64-4DB8-BACE-8E97DF9FA7FD}";
}
			...
			if ($rs = $conn->Execute($sSql)) {
				if (!$rs->EOF) {
					...
					if ($ValidateUser) {
						...
						if (is_null($rs->fields('rol'))) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID(intval($rs->fields('rol'))); // Load User Level
						}
						$this->SetUpUserLevel();
